...
#+BEGIN_SRC sql
  -- 创建公司表
  create table company_result as
    select distinct t.company_id         as cid,
                    ...
                    t.financestage
    from lagou_position_result t;

  -- 删除原表字段
  alter table lagou_position_result drop company_size;
  alter table lagou_position_result drop company_full_name;
  alter table lagou_position_result drop company_short_name;
  alter table lagou_position_result drop company_id;
  alter table lagou_position_result drop financestage;
#+END_SRC
